...
> **SUPERSEDED.** This plan's architecture (Dexie/IndexedDB, a background sync queue, service-worker shell caching) was fully implemented, then removed after real-device testing kept surfacing the same root problem in new places — see the v3 note below for the last and biggest example. It's been replaced entirely by `docs/plans/Offline_Export_Import_plan.md`'s deliberately disconnected export/import design. Kept here as a record of what was tried and why it didn't hold up, not as a description of the current codebase.
...
>
> **Revision note (v3) — the navigation-chain gap:** v2 was built and shipped, and real-device testing surfaced a gap v2's scope missed entirely: caching the survey made the *data* available offline, but nothing cached what a technician actually navigates *through* to reach it. The Field Dashboard's job list (`useMyInspections`) and the lead detail page's own shell (`useLead`, `useLeadProperties`) are each separate, online-only queries with no offline data of their own — offline, the dashboard showed no jobs at all, and even a direct link to a lead showed a query error before `OcularInspectionTab` ever mounted. `downloadSurveyForOffline` now also caches the lead and property records (two new Dexie tables, `leads`/`properties`, added via a proper `.version(2)` migration so it's additive on devices that already have a v1 database); `FieldDashboard` and the lead detail page both fall back to that cache the same way `OcularInspectionTab` already did. Also fixed in the same pass: `navigator.onLine` can report "online" on Android WebView while the device is genuinely unreachable, so the report-save and photo-upload paths now catch a network failure during an "online" attempt and fall back to the offline queue instead of surfacing a raw fetch error with no recovery — and the sync engine's own retry-vs-terminal classification had the same blind spot (`instanceof TypeError` broke the moment an error got wrapped), now fixed to check the message instead.
