...
export default function ScheduleSurveyDialog({ open, onClose, leadId, propertyId }: Props) {
    ...
    const surveyors = users?.filter((u) => ["surveyor", "admin"].includes(u.role)) ?? [];
    ...
    async function onSubmit(values: FormValues) {
        try {
            ...
            toast.error(e instanceof Error ? e.message : "Failed to schedule survey");
        }
    }
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit" disabled={form.formState.isSubmitting}>
                                {form.formState.isSubmitting ? "Scheduling…" : "Schedule Survey"}
                            </Button>
                        </DialogFooter>
                    </form>
                </Form>
